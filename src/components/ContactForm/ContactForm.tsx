...
const schema = yup.object({
    ...
    email: yup.string().email("Некоректний емейл").required("Будь ласка, введіть ваш емейл"),
        ...
        <section id="contact" className="py-20 bg-warm-white overflow-hidden">
            <Container>
                <div
                    ...
                    className={`max-w-3xl mx-auto bg-muted-green p-8 md:p-12 rounded-3xl shadow-xl border-2 border-sage-green/20 transition-all duration-1000 ease-[var(--ease-spring)] ${formInView ? "opacity-100 translate-y-0" : "opacity-0 translate-y-12"
                        }`}
                >
                    <div className="text-center mb-10">
                        <h3 className={`text-3xl font-bold text-warm-brown mb-4 transition-all duration-700 delay-100 ${formInView ? "opacity-100 translate-y-0" : "opacity-0 translate-y-4"}`}>
                            Напишіть нам
                        </h3>
                        <p className={`text-dark-text transition-all duration-700 delay-200 ${formInView ? "opacity-100 translate-y-0" : "opacity-0 translate-y-4"}`}>
                            Маєте запитання або пропозиції? Будемо раді почути вас!
                        </p>
                    </div>

                    <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
                        <div className={`transition-all duration-700 delay-300 ${formInView ? "opacity-100 translate-y-0" : "opacity-0 translate-y-4"}`}>
                        ...
                        <div className={`transition-all duration-700 delay-400 ${formInView ? "opacity-100 translate-y-0" : "opacity-0 translate-y-4"}`}>
                            <label className="block text-sm font-semibold text-warm-brown mb-2 uppercase tracking-wide">
                                Електронна пошта
                            </label>
                            <input
                        ...
                        <div className={`transition-all duration-700 delay-500 ${formInView ? "opacity-100 translate-y-0" : "opacity-0 translate-y-4"}`}>
                            <label className="block text-sm font-semibold text-warm-brown mb-2 uppercase tracking-wide">
                                Повідомлення
                            </label>
                            <textarea
                        ...
                        <div className={`transition-all duration-700 delay-600 ${formInView ? "opacity-100 translate-y-0" : "opacity-0 translate-y-4"}`}>
                            <button
                                type="submit"
