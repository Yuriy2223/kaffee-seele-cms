      ...
      try {
        ...
        if (Array.isArray(data)) {
          ...
          data.forEach((event: any) => {
            ...
            newFormatted[event._id || event.id] = date.toLocaleDateString("uk-UA", {
              day: "numeric",
              month: "long",
              weekday: "long",
            });
      ...
      <section id="events" className="relative bg-sage-green/10">
        ...
        <Container className="px-4 py-10">
          <div
            ...
            className={`text-center mb-16 transition-all duration-700 ${headingVisible ? "opacity-100 translate-y-0" : "opacity-0 translate-y-6"
              }`}
          >
          ...
          >
            {(events.length > 0 ? events : upcomingEvents).map((event: any, index) => {
              const IconComponent =
              ...
              return (
                <div
                  key={event.id}
                  className={`group bg-white rounded-2xl p-8 shadow-lg hover:shadow-2xl hover:scale-[1.03] hover:animate-glow transition-all duration-1000 ease-[var(--ease-spring)] backface-hidden ${cardsVisible ? "opacity-100 translate-y-0 rotate-x-0" : "opacity-0 translate-y-12 -rotate-x-90"
                    }`}
                  style={{ transitionDelay: cardsVisible ? `${index * 250}ms` : "0ms" }}
                >
                  ...
                  <div className="mb-6">
                    ...
                    <div className="w-full bg-cream rounded-full h-2">
                      <div
                        ...
                        style={{ width: cardsVisible ? `${progressPercentage}%` : "0%" }}
                      />
                    </div>
                  </div>
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    <button
                      className={`px-6 py-3 rounded-full font-medium transition-all duration-300 ${event.currentParticipants >= event.maxParticipants
