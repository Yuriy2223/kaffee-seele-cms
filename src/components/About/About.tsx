...
import { Leaf, Heart, Users, Coffee, LucideIcon } from "lucide-react";
...
export const About = () => {
  ...
  const { ref: photosRef, inView: photosVisible } = useInView({ threshold: 0.1 });
  const { ref: cardsRef, inView: cardsVisible } = useInView({ threshold: 0.05 });
  const { ref: valuesRef, inView: valuesVisible } = useInView({ threshold: 0.1 });
    ...
    <section id="about" className="relative bg-cream/10">
      ...
      <Container className="px-4 py-10">
        <div className="max-w-6xl mx-auto text-center">
            ...
            <div className="mb-10">
              ...
              <div className="space-y-6">
                {about.story.map((para, idx) => (
                  <p key={idx} className="text-lg text-dark-text leading-relaxed">
                    {para.text}
                  </p>
                ))}
              </div>
          ...
          >
            {about.images.map((photo, index) => (
              <div
                key={index}
                className={`group overflow-hidden rounded-2xl shadow-lg transition-all duration-1000 ease-out ${photosVisible ? "opacity-100 scale-100 blur-0" : "opacity-0 scale-110 blur-md"
                  }`}
                style={{ transitionDelay: photosVisible ? `${index * 250}ms` : "0ms" }}
              >
                <Image
                  src={photo.url}
            ...
            >
              {storyCards.map((card, index) => {
                ...
                return (
                  <div
                    key={card.title}
                    className={`group bg-white rounded-2xl p-6 shadow-lg hover:shadow-2xl hover:scale-[1.03] transition-all duration-1000 ease-[var(--ease-spring)] backface-hidden ${cardsVisible
                  ...
                  >
                    <div className="flex items-center mb-4">
                      <div className={`w-12 h-12 ${card.bgColor} rounded-full flex items-center justify-center mr-4 transition-transform duration-500 group-hover:rotate-12`}>
                        <Icon className={`w-6 h-6 ${card.iconColor}`} />
                      </div>
                      <h5 className={`font-semibold text-lg ${card.titleColor} group-hover:text-sage-green transition-colors duration-300`}>
                        {card.title}
                      </h5>
                    </div>
          ...
          <div
            ...
            className={`bg-white rounded-2xl p-8 shadow-lg transition-all duration-700 ${valuesVisible ? "opacity-100 translate-y-0" : "opacity-0 translate-y-8"
              }`}
          >
            ...
            <div className="grid md:grid-cols-3 gap-8 text-left">
              {[
                { Icon: Leaf, color: "text-sage-green", title: "Екологічність", text: "Використовуємо органічні інгредієнти та підтримуємо сталий розвиток", delay: 0 },
                { Icon: Heart, color: "text-warm-brown", title: "Якість", text: "Кожна чашка готується з любов'ю та увагою до деталей", delay: 100 },
                { Icon: Users, color: "text-sage-green", title: "Спільнота", text: "Створюємо простір для зустрічей та спілкування", delay: 200 },
              ].map(({ Icon, color, title, text, delay }) => (
                <div
                  key={title}
                  className={`transition-all duration-700 ${valuesVisible ? "opacity-100 translate-y-0" : "opacity-0 translate-y-6"
                    }`}
                  style={{ transitionDelay: valuesVisible ? `${delay}ms` : "0ms" }}
                >
